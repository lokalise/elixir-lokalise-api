defmodule ElixirLokaliseApi.TranslationProvidersTest do
  ...
  test "lists all translation providers" do
    use_cassette "translation_providers_all" do
      {:ok, %TranslationProvidersCollection{} = translation_providers} = TranslationProviders.all @team_id
  ...
  test "lists paginated translation providers" do
    use_cassette "translation_providers_all_paginated" do
      {:ok, %TranslationProvidersCollection{} = translation_providers} = TranslationProviders.all @team_id, page: 2, limit: 1
  ...
  test "finds translation provider" do
    use_cassette "translation_provider_find" do
      provider_id = 3
      {:ok, %TranslationProviderModel{} = provider} = TranslationProviders.find @team_id, provider_id
      ...
      assert provider.description == "Google Translate with neural machine translation support. NMT is not available for all language pairs yet."
